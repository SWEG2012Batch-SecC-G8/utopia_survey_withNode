...
exports.imageUploadPage = (req, res, next) => {
    ...
    var code = req.params.email; //extract course code attached to the URL

    var query2 = `SELECT * FROM customer WHERE email = aaaaaaa@gmail.com `;
    dbConn.query(query2, function(error, result) {
        if (error) {
            console.log(error);
            throw error;
...
exports.uploadImage = (req, res, next) => {
    ...
    upload(req, res, function(err) {
        ...
        var query1 = `SELECT profile_photo_path FROM customer WHERE email = aaaaaaa@gmail.com`;
        var oldImagePath = "";
        ...
        var query2 = `UPDATE customer SET profile_photo_path = "${imgsrc}" WHERE email aaaaaaa@gmail.com"`;
        dbConn.query(query2, function(error, result) {
            if (error) {
            ...
            req.flash("msg", "Image is uploaded.")
            req.flash("title", "Upload Image")
                // return res.redirect("./" + code);
        });
    });
